	...
	int* skip, DoOnItemUInt16 DoOnItem, void* state)
{
	...
	while (!(err = EdfReadBin(&UInt16ValueInf, src, dst, (void **)&s, skip)))
	{
		(*DoOnItem)(s, state);
		s = NULL;
		*skip = 0;
	...
	int* skip, DoOnItemUInt32Fn DoOnItem, void* state)
{
	...
	while (!(err = EdfReadBin(&UInt32ValueInf, src, dst, (void **)&s, skip)))
	{
		(*DoOnItem)(s, state);
		s = NULL;
		*skip = 0;
	...
	int* skip, DoOnItemDoubleFn DoOnItem, void* state)
{
	...
	while (!(err = EdfReadBin(&DoubleValueInf, src, dst, (void **)&s, skip)))
	{
		(*DoOnItem)(s, state);
		s = NULL;
		*skip = 0;
